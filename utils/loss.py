...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
        gt_labels, gt_bboxes, gt_dist = targets.split((1, 4, 1), 2)  # cls, xyxy
        mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)
...
class v8SegmentationLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        try:
            ...
            targets = torch.cat((batch_idx, batch["cls"].view(-1, 1), batch["bboxes"], batch["dist"].view(-1, 1)), 1)
            targets = self.preprocess(targets.to(self.device), batch_size, scale_tensor=imgsz[[1, 0, 1, 0]])
            gt_labels, gt_bboxes, gt_dist = targets.split((1, 4, 1), 2)  # cls, xyxy
            mask_gt = gt_bboxes.sum(2, keepdim=True).gt_(0)
        ...
        _, target_bboxes, target_scores, fg_mask, target_gt_idx, target_dist = self.assigner(
            pred_scores.detach().sigmoid(),
            (pred_bboxes.detach() * stride_tensor).type(gt_bboxes.dtype),
            anchor_points * stride_tensor,
            gt_labels,
        ...
        #dist loss
        target_dist = (target_dist/3).unsqueeze(2)
        loss[4] = self.huber(pred_dist, target_dist)
